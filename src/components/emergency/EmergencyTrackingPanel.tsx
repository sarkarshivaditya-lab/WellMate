...
export function EmergencyTrackingPanel() {
  ...
  const callEmergencyServices = async () => {
    ...
    if (result === "REQUIRES_USER") {
      setCallMessage("Your device opened the emergency call flow; confirm the call on the system dialer.");
    } else if (result === "UNAVAILABLE") {
      setCallMessage("Emergency calling is not configured for this build. Use the SOS alert or configured emergency contacts.");
    } else {
      setCallMessage("Emergency calling is supported by this build.");
    }
  };
      ...
      <Card className="glass-primary border-primary/20">
        <CardHeader className="pb-3">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <CardTitle className="text-base">Golden Hour emergency monitoring</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">Bridging the gap between accident and action.</p>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {coordinates && (
            <div className="glass-subtle flex items-start gap-3 rounded-2xl">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">{coordinates.latitude.toFixed(5)}, {coordinates.longitude.toFixed(5)}</p>
              </div>
            </div>
          )}

          {snapshot.error && (
            <div className="flex items-start gap-3 rounded-2xl border border-destructive/20 bg-destructive/[0.04] px-4 py-3">
              <AlertTriangle className="mt-0.5 h-4 w-4 text-destructive" />
              <p className="text-xs leading-relaxed text-destructive">{snapshot.error}</p>
            </div>
          )}

          {snapshot.state === "CONFIRMATION_WINDOW" ? (
            <div className="space-y-4 rounded-3xl border-2 border-destructive/40 bg-destructive/[0.06] p-5">
              <div className="text-center">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-destructive">Possible accident detected</p>
                <p className="mt-2 text-3xl font-black text-foreground">{seconds}s</p>
                <p className="mt-1 text-sm text-muted-foreground">Tell WellMate you are okay before the timer expires.</p>
              </div>
              <button type="button" onClick={() => void userIsOk()} className="min-h-16 w-full rounded-2xl bg-emerald-600 px-5 text-lg font-extrabold text-white shadow-sm focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-600/30">I'M OK</button>
              <button type="button" onClick={() => void userIsNotOk()} className="min-h-14 w-full rounded-2xl border-2 border-destructive bg-background px-5 text-base font-bold text-destructive focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-destructive/20">I NEED HELP NOW</button>
            </div>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={() => void start("AUTOMATIC")} className="min-h-14 rounded-2xl bg-primary px-4 text-sm font-bold text-primary-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25">Start automatic tracking</button>
              <button type="button" onClick={() => void start("MANUAL")} className="min-h-14 rounded-2xl border border-primary/30 bg-background px-4 text-sm font-bold text-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20">Start manual tracking</button>
            </div>
          )}

          {snapshot.trackingActive && snapshot.state !== "CONFIRMATION_WINDOW" && (
          ...
          {(snapshot.state === "ESCALATED" || snapshot.lastEscalation) && (
            <button type="button" onClick={() => void callEmergencyServices()} className="min-h-14 w-full rounded-2xl border-2 border-amber-500/40 bg-amber-500/[0.06] px-4 text-sm font-extrabold text-amber-700 dark:text-amber-300 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-500/20">
              <span className="inline-flex items-center gap-2"><PhoneCall className="h-4 w-4" />Call emergency services</span>
            </button>
          )}
          ...
          {snapshot.lastEscalation && (
            <div className="glass-subtle space-y-2 rounded-2xl px-4 py-3">
              ...
              {snapshot.lastEscalation.deliveries.map((delivery) => (
                <div key={delivery.contactId} className="flex items-center justify-between gap-3 text-xs">
                  <span className="text-muted-foreground">Contact {delivery.contactId}</span>
