...
function stateLabel(state: ReturnType<typeof useEmergencyTracking>["snapshot"]["state"]): string {
  switch (state) {
    ...
    case "SUSPICIOUS_MOTION":
      return "ANALYSING MOVEMENT";
    case "POSSIBLE_ACCIDENT":
      return "POSSIBLE ACCIDENT";
    case "CONFIRMATION_WINDOW":
      return "ARE YOU OK?";
    case "ESCALATING":
      return "ESCALATING EMERGENCY";
    case "ESCALATED":
    ...
    default:
      return "READY FOR GOLDEN HOUR MONITORING";
  }
}

export function EmergencyTrackingPanel() {
  const { snapshot, start, stop, userIsOk, userIsNotOk, manualEmergency } = useEmergencyTracking();
  ...
  useEffect(() => {
    ...
    const update = () => {
      setCountdown(Math.max(0, snapshot.confirmationDeadlineAt! - Date.now()));
    };
    update();
      ...
      <Card className="border-primary/20 bg-primary/[0.04]">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Bridging the gap between accident and action.
              </p>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-2xl border border-border bg-background/60 px-4 py-4">
            ...
            {snapshot.mode && (
              <p className="mt-1 text-xs text-muted-foreground">Mode: {snapshot.mode === "AUTOMATIC" ? "Automatic" : "Manual"}</p>
            )}
          </div>
          ...
          {snapshot.state === "CONFIRMATION_WINDOW" ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <button
                ...
                className="min-h-14 rounded-2xl bg-primary px-4 text-sm font-bold text-primary-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25"
              >
              ...
              <button
                ...
                className="min-h-14 rounded-2xl border border-primary/30 bg-background px-4 text-sm font-bold text-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20"
              >
          ...
          {snapshot.trackingActive && snapshot.state !== "CONFIRMATION_WINDOW" && (
            <button
              type="button"
              onClick={() => void stop()}
              className="min-h-12 w-full rounded-2xl border border-border bg-background px-4 text-sm font-semibold text-muted-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/15"
            >
              Stop tracking
            </button>
          )}
          ...
          {snapshot.state === "ESCALATED" && (
            <div className="rounded-2xl border border-primary/20 bg-primary/[0.04] px-4 py-4 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Delivery is shown explicitly below. WellMate never assumes that an emergency message was received when the platform cannot confirm delivery.
              </p>
            </div>
          )}
