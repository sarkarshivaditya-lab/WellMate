...
function ResultItem({ result, onSelect }: { result: SearchResult; onSelect: (r: SearchResult) => void }) {
  return (
    <CommandItem key={result.id} value={result.id} onSelect={() => onSelect(result)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer">
      <span className="flex-shrink-0 text-muted-foreground"><ModuleIcon module={result.module} /></span>
      <div className="flex-1 min-w-0"><p className="text-sm font-medium leading-tight truncate">{result.title}</p>{result.subtitle && <p className="text-[11px] text-muted-foreground truncate mt-0.5">{result.subtitle}</p>}</div>
      {result.timestamp && <span className="text-[10px] text-muted-foreground flex-shrink-0">{relativeTime(result.timestamp)}</span>}
    </CommandItem>
  );
}
...
export function WellmateCommandPalette() {
  ...
  const handleQuickAction = useCallback((entity: QuickEntity) => {
    ...
    emitAnalyticsEvent({ type: "command_palette_quick_action", entity, ts: Date.now() });
  }, [closePalette]);

  return (
    <>
      <CommandDialog open={open} onOpenChange={(next) => (next ? undefined : closePalette())}>
        <CommandInput placeholder="Search WellMate..." value={query} onValueChange={setQuery} />
        <CommandList>
          ...
          {filteredNav.length > 0 && (
            <CommandGroup heading="Navigate">
              {filteredNav.map((result) => (
                <CommandItem key={result.id} value={result.title} onSelect={() => handleSelectResult(result)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer">
                  <span className="flex-shrink-0 text-muted-foreground">{NAV_ICONS[result.id]}</span>
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium leading-tight truncate">{result.title}</p><p className="text-[11px] text-muted-foreground truncate mt-0.5">{result.subtitle}</p></div>
                </CommandItem>
              ))}
            </CommandGroup>
          )}
          ...
          {QUICK_ACTIONS.length > 0 && (
            <CommandGroup heading="Quick add">
              {QUICK_ACTIONS.map((action) => (
                <CommandItem key={action.id} value={action.label} onSelect={() => handleQuickAction(action.entity)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer">
          ...
          {[...groupedResults.entries()].map(([module, results]) => (
            <CommandGroup key={module} heading={MODULE_LABELS[module]}>
              {results.map((result) => <ResultItem key={result.id} result={result} onSelect={handleSelectResult} />)}
            </CommandGroup>
          ))}
          ...
          {recentItems.length > 0 && !query.trim() && (
            <>
              ...
              <CommandGroup heading="Recent">
                {recentItems.map((result) => <ResultItem key={result.id} result={result} onSelect={handleSelectResult} />)}
              </CommandGroup>
            </>
          )}
        </CommandList>
      ...
      <QuickAddSheet open={quickAddOpen} onOpenChange={setQuickAddOpen} entity={quickAddEntity} />
    </>
  );
}
